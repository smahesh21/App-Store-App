...
class AppStore extends Component {
  ...
  clickOnTab = tabValue => {
    this.setState({isActiveTabId: tabValue})
  }

  getAppList = () => {
    const {isActiveTabId} = this.state
    const appListWithCategory = appsList.filter(
      eachApp => eachApp.category === isActiveTabId,
    )
    return appListWithCategory
  }

  render() {
    const {searchInput, isActiveTabId} = this.state
    const appListWithCategory = this.getAppList()
    const searchResults = appListWithCategory.filter(eachApp =>
        ...
        <ul className="tabs-container">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              key={eachTab.tabId}
              isActive={isActiveTabId === eachTab.tabId}
            />
          ))}
        </ul>
